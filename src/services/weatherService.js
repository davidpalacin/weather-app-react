...
WeatherService.getWeatherByLocation = async (location) => {
  const result = await axios.get(`http://api.weatherapi.com/v1/forecast.json?key=${import.meta.env.VITE_WEATHER_API}&q=${location}&lang=es
`);
  return result.data;
}
...
WeatherService.getWeatherByLocationAndDay = async (location, day) => {
  const result = await axios.get(`http://api.weatherapi.com/v1/forecast.json?key=${import.meta.env.VITE_WEATHER_API}&q=${location}&days=${day}&lang=es`);
  return result.data;
}
